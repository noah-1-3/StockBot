...
const getCachedData = async <T>(key: string): Promise<T | null> => {
  try {
    ...
    console.error('❌ Error getting cached Supabase data:', error);
    return null;
  }
};
...
const setCachedData = async <T>(key: string, data: T): Promise<void> => {
  try {
    ...
    console.error('❌ Error caching Supabase data:', error);
  }
};
...
export const fetchStockQuoteSupabase = async (symbol: string): Promise<any | null> => {
  try {
    ...
    console.error('❌ Error fetching stock quote from Supabase:', error);
    return null;
  }
};
...
export const searchStocksSupabase = async (query: string): Promise<Array<{ symbol: string; description: string; type: string }>> => {
  try {
    ...
    console.error('❌ Error searching stocks via Supabase:', error);
    return [];
  }
};
...
export const getStockHistoryFromDB = async (symbol: string, days: number = 30): Promise<any[]> => {
  try {
    ...
    if (error) {
      console.error('❌ Error fetching stock history from DB:', error);
      return [];
    }
    ...
    console.error('❌ Error getting stock history from database:', error);
    return [];
  }
};
